...
casper.start('http://localhost/frontend-testing', function () {//http://touzi101admin.vcbrands.com/user
    this.test.assertUrlMatch(/login.php$/, 'Redirected to login page');
    this.test.assertExist("#login_form", 'Login form exists');//form#user-login

    this.fill('#login_form', {//form#user-login
...
casper.thenClick('#login', function () {//id="edit-submit"
    this.test.assertUrlMatch(/index.php$/, 'Redirected to index page after login');//http://touzi101admin.vcbrands.com/contributors/53305
});
